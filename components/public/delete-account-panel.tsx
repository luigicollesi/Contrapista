  ...
  function requestChallenge() {
    ...
    startTransition(async () => {
      ...
      if (!response.ok || !payload.ok || !payload.code || !payload.challengeId) {
        setMessage(payload.message ?? "Não conseguimos gerar o código agora.");
        return;
      }
  ...
  function deleteAccount() {
    ...
    startTransition(async () => {
      ...
      if (!response.ok || !payload.ok) {
        setMessage(payload.message ?? "Não conseguimos concluir a exclusão agora.");
        return;
      }
  ...
  return (
    <section className="mt-10 rounded-sm border border-[#8b1e1e]/45 bg-[#171a1a] p-5 shadow-2xl shadow-black/20 sm:p-6">
      ...
      <p className="mt-3 max-w-2xl text-sm leading-7 text-stone-300">
        Exclua sua conta somente se não quiser mais manter seu nome, partidas e
        progresso no Contrapista.
      </p>
      ...
      {isModalOpen ? (
        ...
        >
          ...
          <p className="mt-4 rounded-sm border border-red-400/35 bg-red-950/25 px-3 py-2 text-sm font-semibold leading-6 text-red-100">
            Digite o código em até 30 segundos para confirmar que essa decisão
            foi intencional.
          </p>
          ...
          <div className="mt-5 rounded-sm border border-[#d0a85c]/25 bg-[#0e1111] p-4">
            ...
            {expiresAt ? (
              <p className="mt-2 text-xs font-semibold text-stone-400">
                Disponível até {formatExpiration(expiresAt)}.
              </p>
            ) : null}
          </div>
